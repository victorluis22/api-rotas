...
class contratoController {
	async create(req, res) {
		...
		db.query(
			...
			(DataIni, DataFim, VolumeBalde, CodTipoContrato, CodEmpresa, CodCliente, CodTipoVeic) 
			VALUES (?, ?, ?, ?, ?, ?, ?)`,
			[
				dataIni,
                dataFim,
    ...
    async readClientContract (req, res){
        const { id } = req.params;
	...
	async update(req, res) {
		...
		db.query(
			`UPDATE contrato SET DataIni=?, DataFim=?, VolumeBalde=?, CodTipoContrato=?, CodEmpresa=?, CodCliente=?, CodTipoVeic=? WHERE NumContrato=?`,
			[
                dataIni,
                dataFim,
                volumeBalde,
